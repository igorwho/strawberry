...
import (
	...
	"github.com/igorwho/strawberry/chatservice/internal/domain/entity"
	"github.com/igorwho/strawberry/chatservice/internal/domain/gateway"
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	if err != nil {
		if err.Error() == "chat not found" {
			...
			if err != nil {
				return nil, errors.New("error create new chat: " + err.Error())
			}
		}
	...
	for {
		...
		if err != nil {
			return nil, errors.New("error receiving chat completion stream: " + err.Error())
		}
...
func createNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	initialMessage, error := entity.NewMessage("system", input.Config.InitialSystemMessage, model)
	if error != nil {
		return nil, errors.New("error creating initial message: " + error.Error())
	}
	chat, error := entity.NewChat(input.UserID, initialMessage, chatConfig)
	if error != nil {
		return nil, errors.New("error creating new chat: " + error.Error())
	}
	return chat, nil
}
